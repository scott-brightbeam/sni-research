 ...
 *
 * Scoring modes:
 *   LLM mode  - uses Anthropic claude-sonnet-4 (requires ANTHROPIC_API_KEY in .env)
 ...
 *   bun scripts/score.js --week 9                  # ISO week 9 of current year
 *   bun scripts/score.js --week 9 --year 2026
...
const RATE_LIMIT_MS = 500; // Sonnet needs slightly more breathing room
...
// Load score prompt once at module level
let scorePromptCache = null;
function getScorePrompt() {
  if (!scorePromptCache) scorePromptCache = loadPrompt('score');
  return scorePromptCache;
}

/**
 * Load the first ~1500 chars of article body from the .md file.
 * Strips YAML frontmatter (between --- delimiters) before extracting.
 * Returns empty string if .md file doesn't exist or can't be read.
 */
function loadArticleBody(mdPath, maxChars = 1500) {
  try {
    if (!existsSync(mdPath)) return '';
    const raw = readFileSync(mdPath, 'utf8');
    // Strip YAML frontmatter
    const fmEnd = raw.indexOf('---', raw.indexOf('---') + 3);
    const body = fmEnd !== -1 ? raw.slice(fmEnd + 3).trim() : raw.trim();
...
async function llmScore(article) {
  ...
  const body = loadArticleBody(article._mdPath);

  const { meta, template } = getScorePrompt();
  ...
  try {
    const response = await anthropic.messages.create({
      ...
      max_tokens: meta.max_tokens || 100,
      messages: [{ role: 'user', content: prompt }],
    });

    if (!response.content?.length) throw new Error('Empty response content');
...
function moveToReview(article, reason, dryRun, stats) {
  ...
  // Move .json and .md together — if .json fails, skip the article entirely
  try {
    ...
    warn(`Failed to move article "${article.title?.slice(0, 50)}": ${err.message}`);
    return;
  }

  // Move raw HTML if it exists (non-critical)
  ...
  // Write reason file alongside moved article (non-critical)
  ...
  } catch { /* reason file is non-critical */ }
...
export async function runScore(args = {}) {
  ...
  if (articles.length === 0) {
    ...
    return { total: 0, kept: 0, moved: 0, wouldMove: 0, byReason: {} };
  }
  ...
  for (const sector of allSectors) {
    ...
    for (const article of sectorArticles) {
      ...
      if (!result.relevant || result.confidence === 'low') {
        ...
        // Persist confidence and reason to article JSON for downstream use (draft.js)
        if (!dryRun && existsSync(article._jsonPath)) {
          try {
            article.confidence = result.confidence;
            article.score_reason = result.reason;
            // Strip internal path fields before writing
            const toWrite = { ...article };
            delete toWrite._jsonPath;
            delete toWrite._mdPath;
            delete toWrite._rawHtmlPath;
            delete toWrite._dateDir;
            delete toWrite._sectorDir;
            writeFileSync(article._jsonPath, JSON.stringify(toWrite, null, 2));
          } catch (err) {
            warn(`Failed to persist score for "${title}": ${err.message}`);
          }
        }
...
if (import.meta.main) {
  ...
  runScore(args)
    .then(stats => {
      log(`Result: total=${stats.total} kept=${stats.kept} moved=${stats.moved}`);
      process.exit(0);
    })
    .catch(e => {
